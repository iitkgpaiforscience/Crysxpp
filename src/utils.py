...
def queryMPDatabse(input_filepath, properties, sample_fraction=1, fetch_cif=True, fetch_mpid=True, print_data=False):
	...
	while end_idx <= len(materials_id_list):
		print(start_idx)
		print(end_idx)
		materials_id=materials_id_list[start_idx:end_idx]
		with MPRester(API_KEY) as m:
			print("Querying Materials Project Database...")
			query = {"material_id": {"$in": materials_id}}
			if fetch_cif:
				properties.append("cif")
			if fetch_mpid:
				properties.append("material_id")
			dataset.extend(m.query(query, properties))
		start_idx=end_idx
		end_idx = start_idx + batch_size
		if final==1:
			break
		if end_idx > len(materials_id_list):
			end_idx=len(materials_id_list)-1
			final=1
...
def writeData(idprop_list, cif_list, material_id_hash_list):
	...
	for row in cif_list:
		...
		count = count + 1
	print("Written Data Total", count)
